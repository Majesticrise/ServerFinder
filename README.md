...
- **Alter deduplication logic / 调整去重策略**: Modify `ResultConsumer.deduplicateFile()` to change the deduplication key (currently `IP:port`).  
  修改 `ResultConsumer.deduplicateFile()` 中的去重键（当前为 `IP:端口`）。
